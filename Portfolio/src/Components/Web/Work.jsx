...
const Work = () => {
  ...
  return (
    <section className='relative min-h-screen bg-[#020409] px-4 pt-[8.2rem] pb-4 text-[#f3f5fb]'>
      <div className='mx-auto w-full max-w-[1220px] rounded-[20px] border border-[#24314e] bg-[linear-gradient(160deg,rgba(9,14,28,0.86)_0%,rgba(7,11,22,0.92)_100%)] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_20px_50px_rgba(0,0,0,0.45)]'>
        ...
        {!selectedCategory ? (
          <div className='mt-5 grid grid-cols-2 gap-3 max-[720px]:grid-cols-1'>
            <button type='button' onClick={() => setSelectedCategory('React Projects')} className='rounded-2xl border border-[#2d426d] bg-[rgba(7,12,24,0.88)] p-5 text-left text-[#edf3ff] transition hover:-translate-y-1 hover:border-[#4b71ba] hover:shadow-[0_10px_24px_rgba(47,114,255,0.22)]'>
              <h3 className='m-0 text-[1.35rem]'>React Projects</h3>
              <p className='mt-2 text-[#aeb6cb]'>Web applications built with React.js</p>
            </button>
            <button type='button' onClick={() => setSelectedCategory('Flutter Apps')} className='rounded-2xl border border-[#2d426d] bg-[rgba(7,12,24,0.88)] p-5 text-left text-[#edf3ff] transition hover:-translate-y-1 hover:border-[#4b71ba] hover:shadow-[0_10px_24px_rgba(47,114,255,0.22)]'>
          ...
          <>
            <button type='button' onClick={() => setSelectedCategory(null)} className='mt-5 rounded-[10px] border border-[#324b7a] bg-[rgba(10,16,31,0.92)] px-4 py-2.5 font-semibold text-[#ecf2ff] transition hover:-translate-y-px hover:border-[#4e74bc]'>Back to categories</button>
            <div className='mt-4 grid grid-cols-3 gap-3 max-[1100px]:grid-cols-2 max-[720px]:grid-cols-1'>
              {filteredProjects.map((project) => (
                <Link to={`/work/${project.slug}`} key={project.slug} className='no-underline'>
                  <article className='h-full rounded-2xl border border-[#27395f] bg-[rgba(8,13,26,0.78)] p-3 transition hover:-translate-y-1 hover:border-[#3d5c95] hover:shadow-[0_12px_26px_rgba(0,0,0,0.35)]'>
